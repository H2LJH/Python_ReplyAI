...
# 이데이터를 nltk 라이브러리를 통해서 전처리,
# vocab().most)common를 이용해서 가장 자주 사용되는 단어 빈도수 확인
...
#　자주 출현하는 단어 50개를 matplotlob를 통해 그래프로 그리기
# 한글 폰트를 로드해야 깨지지않고 출력됨
...
f =open('selecteord.txt', 'w', encoding='UTF-8')
...
# 이제 디이터를 float로 형 변환 시켜주면 데이터 전처리 과정 끝
print('>> 형변환 시작')
...
# - 차레대로 층을 쌓아 만든 모델을 tf.keras.Squential 모델
